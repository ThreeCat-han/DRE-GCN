...
class DREGCN(TagSampleRecommender):
    ...
    def forward(self, row_emb, col_emb, adj_mat, layers=2,perturbed=False):
        all_embeddings = self.get_ego_embeddings(row_emb, col_emb)
        lightgcn_all_embeddings = 0
        # simGCL
        for _ in range(layers):
            all_embeddings = torch.sparse.mm(adj_mat, all_embeddings)
            if perturbed:
                random_noise = torch.rand_like(all_embeddings).cuda()
                all_embeddings += torch.sign(all_embeddings) * F.normalize(random_noise, dim=-1) * 0.1#self.eps==0.1
    ...
    def InfoNCE(self ,view1, view2, temperature: float, b_cos: bool = True):
        ...
        return -score.mean()
    def cal_cl_loss(self, idx1 ,idx2):

        u_idx = torch.unique(idx1.type(torch.long)).to(self.device)
        i_idx = torch.unique(idx2.type(torch.long)).to(self.device)
        ...
        user_view_1, item_view_1 = self.forward(self.user_embedding.weight + ua_u_emb,
                                          self.item_embedding.weight + ia_i_emb,
                                          self.ui_adj_matrix, self.n_layers,perturbed=True)
        user_view_2, item_view_2 = self.forward(self.user_embedding.weight + ua_u_emb,
                                          self.item_embedding.weight + ia_i_emb,
                                          self.ui_adj_matrix, self.n_layers,perturbed=True)
    ...
    def calculate_loss(self, interaction):
        ...
        # # #新增
        cl_loss = 0.5 * self.cal_cl_loss(interaction[self.USER_ID], interaction[self.ITEM_ID])#self.cl_rate==0.5
        loss = mf_loss + tag_loss + cl_loss#self,reg==0.0001
        return loss
