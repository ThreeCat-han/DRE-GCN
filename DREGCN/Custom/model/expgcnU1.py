...
class DREGCN(TagSampleRecommender):
    ...
    # def forward(self, row_emb, col_emb, adj_mat, layers=2):#原
    # def forward(self, row_emb, col_emb, adj_mat, layers):  # 原
    def forward(self, row_emb, col_emb, adj_mat, flag,layers):  # 改
        ...
        for _ in range(layers):
            ...
            if flag==True:
            #     weights = (F.cosine_similarity(side_new, all_embeddings)) * 0.1 + 1
            #     side_new = torch.einsum('a,ab->ab', weights, side_new)
            # weights = (F.cosine_similarity(side_new, all_embeddings, dim=-1)) * 0.1 + 1
            # side_new = torch.einsum('a,ab->ab', weights, side_new)
                if _ > 4:
    ...
    def calculate_loss(self, interaction):
        ...
        flag1 = False
        ua_u_emb, ua_a_emb = self.forward(self.user_embedding.weight, self.tag_embedding.weight, self.ua_adj_matrix, flag1,self.m_layers)
        ...
        ia_i_emb, ia_a_emb = self.forward(self.item_embedding.weight, self.tag_embedding.weight, self.ia_adj_matrix,flag1,self.m_layers)
        ...
        ui_u_emb, ui_i_emb = self.forward(self.user_embedding.weight + ua_u_emb,
                                          self.item_embedding.weight + ia_i_emb,
                                          self.ui_adj_matrix, flag2,self.n_layers)
        #改end
    ...
    def full_sort_predict(self, interaction):
        ...
        if self.restore_user_ua is None or self.restore_tag_ua is None:
            self.restore_user_ua, self.restore_tag_ua = self.forward(self.user_embedding.weight, self.tag_embedding.weight, self.ua_adj_matrix,False, self.m_layers)
        if self.restore_item_ia is None or self.restore_tag_ia is None:
            self.restore_item_ia, self.restore_tag_ia = self.forward(self.item_embedding.weight, self.tag_embedding.weight, self.ia_adj_matrix, False,self.m_layers)
        ...
        if self.restore_user_e is None or self.restore_item_e is None:
            self.restore_user_e, self.restore_item_e = self.forward(self.user_embedding.weight + self.restore_user_ua,
                                                                    ...
                                                                    self.ui_adj_matrix, True,self.n_layers)
