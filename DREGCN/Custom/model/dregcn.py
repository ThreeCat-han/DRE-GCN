...
class DREGCN(TagSampleRecommender):
    ...
    def forward(self, row_emb, col_emb, adj_mat, layers=2, flag=False):
        ...
        if flag == False:#ua和ia的处理
            ...
            uia_embeddings = torch.cat([ua_u_emb, ia_i_emb], dim=0)
            ...
            for _ in range(layers):
                ...
                weights = (F.cosine_similarity(side_embeddings, uia_embeddings, dim=-1)) * self.alpha + 1
                # # weights2 = (F.cosine_similarity(itemtag_embeddings, item_embeddings, dim=-1)) * 0.1 + 1
                side_embeddings = torch.einsum('a,ab->ab', weights, side_embeddings)
        ...
        if flag == False:#ua和ia的处理
            ...
            # ui_u_emb, ui_i_emb = ui_u_emb,ui_i_emb
    ...
    def calculate_loss(self, interaction):
        ...
        ui_u_emb, ui_i_emb = self.forward(self.user_embedding.weight + ua_u_emb,
                                          ...
                                          self.ui_adj_matrix, self.n_layers,True)#原
